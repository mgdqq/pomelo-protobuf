	...
	while(offset<length){
		...
		switch(protos[name].option){
			...
			case 'map':
				...
				decodeMap(msg[name], protos[name], protos);
			break;
		}
	}
...
function decodeMapArray(array, type, protos) {
	var length = codec.decodeUInt32(getBytes());
	for (var i = 0; i < length; i++) {
		array.push(decodeProp(type, protos));
	}
...
function decodeMap(eMap, proto, protos) {
	const length = codec.decodeUInt32(getBytes());
	const type = proto.type;
	for (let i = 0; i < length; i++) {
		const k = decodeProp(proto.keyType, protos)
		if (typeof type == 'object') {
			const subProto = type;
			if (subProto.option == 'map') {
				eMap[k] = eMap[k] || {};
				decodeMap(eMap[k], subProto, protos);
			} else {
				eMap[k] = eMap[k] || [];
				decodeMapArray(eMap[k], subProto.type, protos);
			}
		} else {
			const v = decodeProp(type, protos);
			eMap[k] = v;
		}
	}
}
