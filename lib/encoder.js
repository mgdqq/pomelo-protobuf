	...
	for(var name in msg){
		if(!!protos[name]){
			...
			switch(proto.option){
				...
				case 'map':
					offset = writeBytes(buffer, offset, encodeTag(proto.type, proto.tag));
					offset = encodeMap(msg[name], proto, offset, buffer, protos);
					break;
			}
		}
	}
...
function encodeMapArray(array, proto, offset, buffer, protos) {
	var i = 0;
	offset = writeBytes(buffer, offset, codec.encodeUInt32(array.length));
	for (i = 0; i < array.length; i++) {
		offset = encodeProp(array[i], proto.type, offset, buffer, protos);
...
function encodeMap(eMap, proto, offset, buffer, protos) {
	const keys = Object.keys(eMap);
	offset = writeBytes(buffer, offset, codec.encodeUInt32(keys.length));
	for (let k in eMap) {
		offset = encodeProp(k, proto.keyType, offset, buffer, protos);

		if (typeof proto.type == 'object') {
			const subProto = proto.type;
			if (subProto.option == 'map') {
				offset = encodeMap(eMap[k], subProto, offset, buffer, protos);
			} else {
				offset = encodeMapArray(eMap[k], subProto, offset, buffer, protos);
			}
		} else {
			offset = encodeProp(eMap[k], proto.type, offset, buffer, protos);
		}
	}
